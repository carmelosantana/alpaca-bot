...
## Subagent models (decided 2026-09-15)

All Opus for now: implementer, fixer and reviewer. No Fable, never Haiku or Sonnet. Pass `model: "opus"` on every dispatch, and seed tickets with `implementer_model: opus` and `reviewer_model: opus`. Older tickets and plans that name `fable` predate this.
