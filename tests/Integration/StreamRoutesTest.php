 ...
 * real permission callback and transient store, and stream() runs the real pipeline over real
 * posts with the provider faked. What cannot run here is serve(), the rest_pre_serve_request
 * side: Sse::prepareOutput() ends every output buffer, PHPUnit's included, so the wire itself
 * is the real check's (curl against the harness site).
 *
 * @group rest
 */
final class StreamRoutesTest extends TestCase
{
    ...
    public function test_the_ticket_is_redeemed_once_by_its_owner_for_its_conversation_only(): void
    {
        $owner = $this->asAdmin();
        [$token] = $this->ticket();
        ...
        $this->assertIsArray(get_transient('alpaca_bot_stream_' . $token));
        ...
        $this->assertSame(['X-Alpaca-Bot-Stream' => '1'], $res->get_headers());
        $this->assertSame($owner, $res->get_data()['user_id']);
        $this->assertSame('later', $res->get_data()['message']);
        $this->assertSame(0, $res->get_data()['options']['conversation_id']);
    ...
    public function test_stream_writes_start_deltas_and_done_and_persists_the_turn(): void
    {
        ...
        $ticket = $this->rest('GET', '/chat/0/stream', ['token' => $token])->get_data();
    ...
    public function test_a_refused_turn_is_a_single_error_frame(): void
    {
        ...
        $ticket = $this->rest('GET', "/chat/{$other}/stream", ['token' => $token])->get_data();
        $frames = $this->frames($ticket);
