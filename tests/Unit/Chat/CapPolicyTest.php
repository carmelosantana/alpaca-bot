...
/** Serves the cached month summaries the meter would find, keyed by scope. */
function usageCache(int $user, int $site): void
{
...
function policy(array $settings): CapPolicy
{
...
it('allows under both caps and offers each verdict to the filter with its scope, limit and usage', function (): void {
    ...
    policy(['governance.user_monthly_tokens' => 100, 'governance.site_monthly_tokens' => 1000])->assertAllowed(3);
    expect(true)->toBeTrue();
});
...
it('throws for the user cap before the site cap is even read', function (): void {
    ...
    $policy = policy(['governance.user_monthly_tokens' => 100, 'governance.site_monthly_tokens' => 1000]);
    try {
        $policy->assertAllowed(3);
...
it('throws for the site cap when the user is unlimited and usage exactly meets the limit', function (): void {
    ...
    try {
        ...
        expect($e->scope)->toBe('site')
            ...
            ->and($e->getMessage())->toBe('The site\'s monthly token cap has been reached (1000 of 1000 tokens).');
    }
});
...
it('lets the filter override a block', function (): void {
    ...
    policy(['governance.site_monthly_tokens' => 10])->assertAllowed(3);
    expect(true)->toBeTrue();
});
...
it('lets the filter impose a block on a request that is under the cap', function (): void {
    ...
    expect(fn() => policy(['governance.user_monthly_tokens' => 100, 'governance.site_monthly_tokens' => 1000])->assertAllowed(3))
...
it('bounds an anonymous request by the site cap alone', function (): void {
    ...
    expect(fn() => policy(['governance.user_monthly_tokens' => 100, 'governance.site_monthly_tokens' => 10])->assertAllowed(0))
